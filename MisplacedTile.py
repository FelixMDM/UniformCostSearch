...
def misplaced_tiles_heuristic(currState):
    totalCost = 0
...
# A* search algorithm with Misplaced Tile heuristic
        ...
        if currentNode.get_data() not in [node.get_data() for node in visited]:
            ...
            for node in [rightNode, leftNode, upNode, downNode]:
                if node:
                    ...
                    cost = misplaced_tiles_heuristic(node.get_data())
                    node.set_heuristic(cost)
